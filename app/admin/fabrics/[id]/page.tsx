...
import { FABRIC_CATEGORIES, type FabricCategory, type Fabric } from "@/types/fabric";
...
export default function EditFabricPage() {
  ...
  async function fetchFabric() {
    try {
      ...
      const fabric: Fabric = data.fabric;

      setName(fabric.name);
      setCategory(fabric.category);
      setDescription(fabric.description);
      setColorHex(fabric.colorHex || "#808080");
      setTags(fabric.tags?.join(", ") || "");
      setCurrentImageUrl(fabric.imageUrl);
      setImagePreview(fabric.imageUrl);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load fabric");
    } finally {
      setIsFetching(false);
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to update fabric");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <Link href="/admin/fabrics" className="mt-4 inline-block">
              <Button variant="outline">Back to Fabrics</Button>
            </Link>
          </CardContent>
        </Card>
      </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-xs text-[var(--color-text-muted)]">
                    {imageFile
                      ? "New image selected"
                      : "Leave unchanged or select new image"}
                  </p>
                </div>
              </div>
            </div>
